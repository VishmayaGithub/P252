...
cred = credentials.Certificate("potentiometer-9b6ea-firebase-adminsdk-d0mo1-291d5295b3.json")
...
def index():
    try:
        ...
        pot = db_client.collection('Potentiometer').document('potentiometer values').get().to_dict()
...
def add():
    try:
        ...
        db_client.collection('Potentiometer').document('potentiometer values').set({'pot val' : pot_val})
